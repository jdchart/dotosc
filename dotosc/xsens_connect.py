...
class XSConnect:
    ...
    async def get_devices(self):
        ...
        for device in devices:
            #print(device)
            if device.name != None:
                if "Movella" in device.name or "Xsens" in device.name or "DOT" in device.name:
    ...
    async def connect_device(self):
        async with BleakClient(self.id) as client:
            ...
            await client.write_gatt_char(self.measurement_char_uuid, self.measurement_mode_string, response = True)
    ...
    async def discover_characteristics(self):
        async with BleakClient(self.id) as client:
            print()
            # Get the services from the connected device
            services = await client.get_services()

            # Iterate over the services to get their characteristics
